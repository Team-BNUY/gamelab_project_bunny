...
namespace AI.Agents
{
    public class Teacher : Agent, IPunObservable
    {
        ...
        private void OnCollisionEnter(Collision collision)
        {
            ...
            if (!_stunned)
            {
                ...
                if (collision.gameObject.TryGetComponent<NetworkSnowball>(out var ball))
                {
                    ...
                    {
                        ScoreManager.IncrementPropertyCounter(PhotonNetwork.LocalPlayer, ScoreManager.RebelKey);
                    }
                }
            }
            ...
            // Hit by a snowball
            var throwerPosition = snowball.StudentThrower.transform.position;
        ...
        private bool WitnessedBadAction(out NetworkStudentController badStudent)
        {
            ...
            {   
                if(!student.HasSnowball && !student.IsUsingCannon && !_badStudents.ContainsKey(student)) continue;
